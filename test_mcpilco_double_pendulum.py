...
import simulation_class.ode_systems as ode
...
model_learning_par["angle_indeces"] = [0, 1]  # Indeces of the state components that are angles
model_learning_par["not_angle_indeces"] = [2, 3]  # Indeces of the state components that are not angles
model_learning_par["T_sampling"] = T_sampling
model_learning_par["vel_indeces"] = [2, 3]  # Indeces of the state components that are velocities
model_learning_par["not_vel_indeces"] = [0, 1]  # Indeces of the GP input components that are not velocities
...
control_policy_par["angle_indices"] = np.array([0, 1])  # Index of theta1 and theta2 inside state vector
control_policy_par["non_angle_indices"] = np.array([2, 3])  # Indeces of [theta_dot1, theta_dot2] inside state vector
...
cost_function_par["lengthscales"] = torch.tensor([3.0, 3.0], dtype=dtype, device=device)
